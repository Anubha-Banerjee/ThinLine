...
public class MainActivity extends Activity
  ...
  private boolean mAutoStartSignInFlow = true;
    ...
    static // Room ID where the currently active game is taking place; null if we're
    // not playing.
    String mRoomId = null;
    ...
    static byte[] mMsgBuf = new byte[16];
  ...
  public void onClick(View v) {
    ...
    public void onRealTimeMessageReceived(RealTimeMessage rtm) {
        ...
        GameActivity.drawOpponentLine(floatArray[0], floatArray[1], floatArray[2], floatArray[3]);   
    }
	...
	private static byte[] toByteArray(float[] floats) {
		byte[] byteArray = new byte[floats.length*4];	
		ByteBuffer byteBuf = ByteBuffer.wrap(byteArray);	
		FloatBuffer floatBuf = byteBuf.asFloatBuffer();		
		floatBuf.put (floats);
		return byteArray;  
	}
    // broadcast the lastly added or modified line to add
    public static void broadcastLine(Line line) {
        if (!mMultiplayer)
            return; // playing single-player mode
        
        float[] lineCoordinates = new float[4];
        lineCoordinates[0] = line.getX1();
        lineCoordinates[1] = line.getY1();
        lineCoordinates[2] = line.getX2();
        lineCoordinates[3] = line.getY2();
        
        mMsgBuf = toByteArray(lineCoordinates);    
